...
public:
    ...
    void calculateRoute(const std::vector<std::pair<double, double>>& waypoints) override {
        std::cout << "Calculating optimized route for ElectricCar using custom algorithm... for " << name << "\n";
        // Custom algorithm for ElectricCar
        size_t n = waypoints.size();
        ...
        std::lock_guard<std::mutex> lock(mtx);
        std::cout << "Optimized Path for " << name << " (ElectricCar):\n";
        for (const auto& point : optimizedPath) {
            std::cout << "(" << point.first << ", " << point.second << ")\n";
        }
    }
...
public:
    ...
    void calculateRoute(const std::vector<std::pair<double, double>>& waypoints) override {
        std::cout << "Calculating optimized route for SUV using custom algorithm... for " << name << "\n";
        // Custom algorithm for SUV
        size_t n = waypoints.size();
        ...
        std::lock_guard<std::mutex> lock(mtx);
        std::cout << "Optimized Path for " << name << " (SUV):\n";
        for (const auto& point : optimizedPath) {
            std::cout << "(" << point.first << ", " << point.second << ")\n";
        }
    }
...
void trackVehicle(Car* vehicle, const std::vector<std::pair<double, double>>& route) {
    try {
        vehicle->calculateRoute(route);
...
int main() {
    ...
    std::thread t1(trackVehicle, &tesla, std::ref(route));
    std::thread t2(trackVehicle, &jeep, std::ref(route));
